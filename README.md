...
For dev / smoke-test on your laptop, set `DEV_ADMIN_LOGIN=<any-login>` and skip the OAuth dance. With this env set, clicking **"Sign in with GitHub"** in the topbar **does not redirect to github.com** — the server short-circuits the `/api/v1/auth/github/start` route, upserts a synthetic user with the given login, sets the session cookie, and bounces back to `/`. Same `is_admin=true` (when the login is also in `ADMIN_GITHUB_LOGINS`) as a real OAuth login.

Startup log will say:

```
[dev] DEV_ADMIN_LOGIN=demo_admin — "Sign in with GitHub" bypasses OAuth and grants this login (is_admin=true).
[dev] WARNING: do NOT set DEV_ADMIN_LOGIN in production.
```

`GITHUB_CLIENT_ID/SECRET` are still required at startup (the server fails fast on missing env), but you can use placeholder values like `stub` — they're never actually used. **Never set `DEV_ADMIN_LOGIN` in production** — it's a hard OAuth bypass.
