...
// 检测方式: 第一个元素有 `steps` 属性 → 已是 group 形态。旧 schema 的 step 对象只含
// {id, num, title, desc}, 不会出现 steps, 所以这个判断在实际数据下无歧义。
export function normalizeTours(tours) {
  if (tours.length === 0 || (tours[0] && typeof tours[0].steps !== 'undefined')) return tours;
  // 旧 schema: 扁平 step 数组. 包成单 group, slug 用 'main', title=null
  // (sidebar.js / content.js 在渲染时检测到 null 会用 T.sidebar_tour_head 兜底)
