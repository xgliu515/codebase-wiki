...
| `templates/web/js/app.js` | `import { T } from './strings.js'`;`main()` 起始处加 `applyI18n()`,把 DOM 上的 `data-i18n="<key>"` 节点的相应属性赋成 `T[key]`;另外 line 52/80/81/137/139-144/147/175 几处硬编码中文字符串(toast、prompt 对话、启动错误页)替换为 `T.*` |
| `templates/web/js/content.js` | `renderHome()` 整段(home hero、推荐 trace、架构总览说明、Trace 导览段头、参考手册段头、键盘快捷键表)所有硬编码中文替换为 `T.*`;`loadChapter()` 的错误/loading 文案(line 59/62/70)替换;`enhanceFileRefs()` verb(line 196)替换;`makeAddendumBanner()` 前缀(line 235)替换;mermaid 占位 `⏳ 渲染中…`(line 21)替换 |
| `templates/web/js/sidebar.js` | `首页` link(line 37)、`单请求 Trace 导览` 段头(line 41)、`参考手册(${n} 章)` 段头(line 49)、aria-label `展开/收起`(line 63)、`无目录` 占位(line 106)替换为 `T.*` |
| `templates/web/js/search.js` | `无结果` 占位(line 127)替换为 `T.search_no_results` |
| `templates/web/js/glossary.js` | line 68 正则改为接受 zh+en label 的 alternation;line 71-74 switch 分支增 `'Original name'`/`'Definition'`/`'Source'` case;line 251/253/258 panel 按钮文案(`‹ 返回`/`关闭 (Esc)`/`重置`/`清除本地"已查看"记录`)替换;line 342/343/346 渲染时用 `T.gloss_*_label`;英文模式下 `T.gloss_chinese_label === ''`,配合现有 `${term.chineseName ? ... : ''}` 三元自动隐藏中文译名行 |
...
- `templates/web/js/{diagrams,utils,versions}.js`:不含语言相关字符串
- `templates/web/js/{chapters,architecture}.js`:含中文是因为它们是 **per-project metadata**(章节标题、layer 名),由 chapter agent 按项目语言生成,不在 i18n strings.js 管辖范围
...
完整 i18n 表(~45 key)在实施 plan 中以 `templates/web/js/strings.js` 的形式给出,涵盖 `index.html` 静态 chrome + `app.js`/`content.js`/`sidebar.js`/`search.js`/`glossary.js` 运行时字符串。下面展示**节选**结构,**不是完整列表**——完整内容见 plan:
...
3. **新模板 en 路径** — 同一 mock 项目(en),浏览器打开后所有 chrome 全英文:**顶栏 chrome、侧栏(首页/段头/aria)、首页 hero+stats+架构图说明+导览段+参考手册段+键盘快捷键表、章节加载/错误页、addendum banner、术语侧栏按钮、搜索无结果占位、app.js toast 和 source-mode prompt 对话**;字体视觉对(SF Pro/Segoe UI 而非 PingFang);章节正文英文;glossary 用英文 schema(`Original name`/`Definition`/`Source`),术语弹窗能开,无「中文译名」行
